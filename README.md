Hi, I'm Mert Efe Kurt 👋  
I'm a **Software Developer** passionate about building efficient, secure, and data-driven systems.  
🎓 Management and Information Systems (MIS) student.  
🛡️ I love exploring the intersection of **data, security, and automation.**

Previously worked on:
[`Data Automation Projects`](#) [`Dockerized Apps`](#) [`Security Research`](#)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Mert%20Efe%20Kurt-blue?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/mertefekurt)
[![GitHub](https://img.shields.io/badge/GitHub-mertefekurt-black?style=flat-square&logo=github)](https://github.com/mertefekurt)

---

| ![](https://github-profile-summary-cards.vercel.app/api/cards/stats?username=mertefekurt&theme=dracula) | ![](https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=mertefekurt&theme=dracula) | ![](https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=mertefekurt&theme=dracula) |
|:-----:|:------:|:------:|

---

# 🧩 Recent Projects

1. **Data Cleaning & Analysis Pipelines (Python + SQL)**
   - Automated scripts for transforming and sanitizing large datasets.
   - Focus on reliability and reproducibility with Dockerized workflows.

2. **Containerized Security Tools (Docker + Cybersecurity)**
   - Built and deployed lightweight containers for network scanning and vulnerability analysis.

3. **Automation Utilities**
   - Small tools that make everyday development faster, safer, and smarter.

---

> _“Code is art, but secure code is craftsmanship.”_

### 🧠 Recent Repositories
<!--START_SECTION:activity-->
<!--END_SECTION:activity-->
